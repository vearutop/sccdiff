...
func Main() {
	...
	if f.groupDirs != "" {
		groupDirs = strings.Split(f.groupDirs, ",")
	}

	current, err := getResults("", groupDirs)
	if err != nil {
		log.Fatal(err)
	...
	do := func(workPath string) {
		...
		base, err := getResults(workPath+"/", groupDirs)
		if err != nil {
			log.Fatal(err)
...
func getResults(stripPath string, groupDirs []string) (map[string]resultRow, error) {
	...
	for i := 1; i < len(s); i++ {
		...
		for _, dir := range groupDirs {
			if strings.HasPrefix(row.Location, dir+"/") {
				row.Language = dir
			}
		}
